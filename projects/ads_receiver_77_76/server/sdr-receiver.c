...
int main(int argc, char *argv[])
{
  ...
  if(argc != 2)
  {
    fprintf(stderr, "Usage: sdr-receiver interface\n");
    return EXIT_FAILURE;
  }
  ...
  else
  {
    ...
    fifo = mmap(NULL, 32*sysconf(_SC_PAGESIZE), PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0x82000000);
  }
  ...
  if(setsockopt(sock_server, SOL_SOCKET, SO_BINDTODEVICE, argv[1], strlen(argv[1]) + 1) < 0)
  {
