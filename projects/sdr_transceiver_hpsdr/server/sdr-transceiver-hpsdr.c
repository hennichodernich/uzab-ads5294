...
const uint32_t freq_max = 38880000;
...
int main(int argc, char *argv[])
{
  ...
  *gpio_out = 0;
  ...
  rx_freq[0] = (uint32_t)floor(600000 / 77.76e6 * (1 << 29) + 0.5);
  rx_freq[1] = (uint32_t)floor(600000 / 77.76e6 * (1 << 29) + 0.5);
  rx_freq[2] = (uint32_t)floor(600000 / 77.76e6 * (1 << 29) + 0.5);

  /* set default rx sample rate */
  *rx_rate = 648;

  /* set default tx phase increment */
  *tx_freq = (uint32_t)floor(600000 / 77.76e6 * (1 << 29) + 0.5);
...
void process_ep2(uint8_t *frame)
{
  ...
  switch(frame[0])
  {
    ...
    case 3:
      ...
      *tx_freq = (uint32_t)floor(freq / 77.76e6 * (1 << 29) + 0.5);
      if(freq_data[0] != freq)
      {
        freq_data[0] = freq;
        icom_write();
    ...
    case 5:
      ...
      freq = ntohl(*(uint32_t *)(frame + 1));
      if(freq < freq_min || freq > freq_max) break;
      rx_freq[0] = (uint32_t)floor(freq / 77.76e6 * (1 << 29) + 0.5);
    ...
    case 7:
      ...
      freq = ntohl(*(uint32_t *)(frame + 1));
      if(freq < freq_min || freq > freq_max) break;
      rx_freq[1] = (uint32_t)floor(freq / 77.76e6 * (1 << 29) + 0.5);
      if(freq_data[2] != freq)
      {
        freq_data[2] = freq;
        alex_update = 1;
    ...
    case 9:
      ...
      freq = ntohl(*(uint32_t *)(frame + 1));
      if(freq < freq_min || freq > freq_max) break;
      rx_freq[2] = (uint32_t)floor(freq / 77.76e6 * (1 << 29) + 0.5);
      break;
#else
    case 6:
    case 7:
