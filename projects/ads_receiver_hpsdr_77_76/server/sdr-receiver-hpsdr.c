...
int main(int argc, char *argv[])
{
  ...
  if(errno != 0 || end == argv[2] || number < 1 || number > 8)
  {
      fprintf(stderr, "Usage: sdr-receiver-hpsdr interface 1|2|3|4|5|6|7|8\n");
      return EXIT_FAILURE;
  }
  chan = (number - 1);
...
void process_ep2(uint8_t *frame)
{
  ...
  switch(frame[0])
  {
    ...
    case 5:
      ...
      rx_freq[0] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 6:
    case 7:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[1] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 8:
    case 9:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[2] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 10:
    case 11:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[3] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 12:
    case 13:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[4] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 14:
    case 15:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[5] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 16:
    case 17:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[6] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
    case 36:
    case 37:
      /* set rx phase increment */
      freq = ntohl(*(uint32_t *)(frame + 1));
      rx_freq[7] = (uint32_t)floor(modf(freq / 77.76e6, &integral) * 0xffffffff + 0.5);
      break;
  }
}
